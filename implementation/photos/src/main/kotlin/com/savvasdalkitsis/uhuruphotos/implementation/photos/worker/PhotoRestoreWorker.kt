...
@HiltWorker
class PhotoRestoreWorker @AssistedInject constructor(
    ...
    override suspend fun doWork() = withContext(Dispatchers.IO) {
        try {
            ...
            if (shouldRestoreLocally(response)) {
                photoRepository.restorePhotoFromTrash(id)
                Result.success()
            } else {
                failOrRetry()
            }
    ...
    private fun shouldRestoreLocally(response: Response<PhotoOperationResponse>) =
