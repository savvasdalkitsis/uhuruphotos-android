...
class LocalMediaResolver @Inject constructor(
    private val localMediaFolderRepository: LocalMediaFolderRepository,
    private val localMediaService: LocalMediaService,
) {

    suspend fun resolveMediaForFolder(folderId: Int) = with(localMediaService) {
        getPhotosForBucket(folderId) + getVideosForBucket(folderId)
    }

    suspend fun resolveAllLocalMediaInOrder(): List<LocalMediaStoreServiceItem> {
        ...
        return cameraItems + photos + videos
    }
}