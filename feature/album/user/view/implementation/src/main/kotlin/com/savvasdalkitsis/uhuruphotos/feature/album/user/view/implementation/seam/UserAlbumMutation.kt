...
sealed class UserAlbumMutation(
    mutation: Mutation<UserAlbumState>,
) : Mutation<UserAlbumState> by mutation {

    data class ShowDeleteConfirmationDialog(val show: Boolean) : UserAlbumMutation({
        it.copy(showDeleteConfirmationDialog = show)
    })

    data class SetAlbumId(val id: Int) : UserAlbumMutation({
        it.copy(albumId = id)
    })
}